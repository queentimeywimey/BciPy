...
This module provides the image display implementation which handles the visual
presentation of image stimuli for image-based BCI paradigms. It provides core
functionality for stimulus presentation, timing control, and inquiry management.
"""
...
class ImageDisplay(Display):
    """Image Display Object for inquiry Presentation.

    Animates display objects common to any image task. Handles stimulus presentation,
    timing control, and inquiry management for image-based BCI paradigms.
    ...
    def __init__(
        ...
        """Initialize image display parameters and objects.
    ...
    def do_inquiry(self) -> List[Tuple[str, float]]:
        """Present an inquiry of sequential image stimuli.
    ...
    def _generate_inquiry(self) -> List[Dict[str, Any]]:
        """Generate stimuli for the next image inquiry.
        ...
        for idx, stim in enumerate(self.stimuli_inquiry):
            ...
            if any([stim.endswith(ext) for ext in [".png", ".bmp", ".jpg"]]):
                ...
                current_stim['sti_label'] = path.splitext(
                    path.basename(stim))[0] # image names should be unique!
            else:
                current_stim['sti'] = self._create_stimulus(
    ...
    def _create_stimulus(
        ...
        if mode == 'text':
            ...
            raise ValueError(
                f'ImageDisplay asked to create a stimulus type=[{mode}] that is not supported.')
